...
export function ResetMonthModal({ monthName, onReset }: ResetMonthModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger 
        render={
          <Button variant="outline" size="sm" className="h-7 text-xs font-medium text-rose-600 bg-rose-50 border-rose-100 hover:bg-rose-100 rounded-full px-3" />
        }
      >
      ...
      <DialogContent className="w-[95vw] sm:max-w-[400px] p-5 sm:p-6 rounded-2xl sm:rounded-2xl max-h-[90vh] overflow-y-auto">
        ...
        <DialogFooter className="flex flex-col-reverse sm:flex-row gap-2 sm:gap-2 mt-2">
          ...
          <Button 
            variant="destructive" 
            onClick={handleReset}
            className="w-full sm:flex-1 rounded-xl h-11 bg-rose-600 hover:bg-rose-700 font-medium"
            disabled={isLoading}
          >
